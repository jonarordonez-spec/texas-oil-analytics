...
|-------|-------------|---------|
| **BRONZE** | Raw data ingestion. Original .dsv files from RRC converted to Parquet. | Parquet |
| **SILVER** | Data cleaning and feature engineering. Standardized data types, handled missing values, created BOE, active flags, and lag variables. | Parquet + DuckDB |
| **GOLD** | Aggregations and modeling sample. Monthly aggregates by district and operator. Consistent 500k row modeling sample. PostgreSQL tables with indexes and materialized views. | Parquet + DuckDB + PostgreSQL |
...
1. **Step 1**: Use ML to predict outcome (Y) from confounders (X)
2. **Step 2**: Use ML to predict treatment (T) from confounders (X)
3. **Step 3**: Compute residuals and estimate causal effect
4. **Step 4**: Use cross-fitting (n_folds=3, n_rep=2) for robustness

### Model Parameters

| Parameter | Value | Description |
|-----------|-------|-------------|
| **ML Model** | Random Forest | Non-linear, robust |
| **N Estimators** | 60 | Number of trees |
| **Max Depth** | 5 | Tree depth |
| **N Folds** | 3 | Cross-fitting folds |
| **N Rep** | 2 | Repetitions |
...
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jonathan-ruiz-ordo%C3%B1ez-001673233/)
</div>
